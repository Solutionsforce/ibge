...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initializeMobileResponsiveFixes();
});
 ...
 */
function initializeMobileResponsiveFixes() {
    // Only run on mobile devices
    if (window.innerWidth <= 768) {
        fixMobileHeaderElements();
        fixMobileLogoSizing();
        fixMobileTextSizing();
        fixMobileSpacing();
        
        // Re-run fixes on screen orientation change
...
/**
 * Fix mobile header elements
 */
function fixMobileHeaderElements() {
    // Fix gov.br logo
    const govLogos = document.querySelectorAll('img[src*="gov.br"]');
    govLogos.forEach(logo => {
        logo.style.height = '20px';
        logo.style.width = 'auto';
        logo.style.minWidth = '60px';
        logo.style.maxWidth = '100px';
    });
    
    // Fix header height
    const headerContainers = document.querySelectorAll('.bg-\\[\\#0B2341\\] .h-\\[42px\\]');
 ...
 */
function fixMobileLogoSizing() {
    // Fix IBGE logos
    const ibgeLogos = document.querySelectorAll('img[src*="ibge"]');
    ibgeLogos.forEach(logo => {
        logo.style.width = '125px';
        logo.style.height = '24px';
        logo.style.minWidth = '125px';
        logo.style.maxWidth = '125px';
    });
    
    // Fix flag images
    const flagImages = document.querySelectorAll('img[src*="pt_br"], img[src*="flagcdn"]');
 ...
 */
function fixMobileTextSizing() {
    // Force consistent text sizes
    const textElements = {
        '.text-xs': '0.75rem',
        '.text-sm': '0.875rem',
        '.text-base': '1rem',
    ...
    Object.keys(textElements).forEach(selector => {
        ...
        elements.forEach(element => {
            element.style.fontSize = textElements[selector];
        });
    });
}
...
function fixMobileSpacing() {
    ...
    Object.keys(spacingClasses).forEach(selector => {
        const elements = document.querySelectorAll(selector);
        elements.forEach(element => {
            const styles = spacingClasses[selector];
