...
- July 4, 2025: Complete redesign of /selecao-local-prova page - Removed complex school search functionality and replaced with simplified address confirmation and consent terms. Users now see their residential address and agree to receive exam location details via email 7 business days before the exam. Removed /api/buscar-escolas endpoint and escola_utils.py dependencies. Streamlined flow from /login → /selecao-cargo → /selecao-local-prova (consent) → /checkout.
- July 4, 2025: Registration flow optimization - Removed /carregamento-verificacao page from navigation flow. Users now go directly from /login (with 5-second loading) to /selecao-cargo, simplifying the application process.
